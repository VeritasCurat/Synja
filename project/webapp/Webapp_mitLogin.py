...
def render_indexSynja_DE():
    return render_template('de/indexSynja_mitLogout.html', async_mode=socketio.async_mode) 
    
@app.route('/synja_en')
def render_indexSynja_EN():
    return render_template('en/indexSynja_mitLogout.html', async_mode=socketio.async_mode) 
...
def render_indexGate():
    return render_template('de/indexGate.html', async_mode=socketio.async_mode)  
...
@socketio.on('logout', namespace='/synja_de')
def logout_synjaDE():
    if(request.remote_addr in logins.keys()):
      logins.pop(request.remote_addr)
      emit('redirect', {'url': url_for('render_mainDE')}) 
      for synja in synjas:
        if synja.id == request.sid:
          synja.save()
          
@socketio.on('logout', namespace='/synja_en')
def logout_synjaEN():
    if(request.remote_addr in logins.keys()):
      logins.pop(request.remote_addr)
      emit('redirect', {'url': url_for('render_mainDE')}) 
