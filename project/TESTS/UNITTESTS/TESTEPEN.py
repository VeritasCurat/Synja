...
class Test(unittest.TestCase):
  ...
  ep = ExpertenmodellEN()
  epDE =  ExpertenmodellDE()
  
  def testkongruenz_DEEN(self):
    for lesson in self.ep.lessons:
      if lesson not in self.epDE.lessons:
        raise AssertionError(lesson+" nicht in DE")
      
    for lesson in self.ep.lessons:
      for konzept in self.ep.lessoninhalte[lesson]:
        if konzept not in self.epDE.lessoninhalte[lesson]:
          raise AssertionError(konzept+" nicht in DE")
        
    for fehlerbeschr in self.ep.fehlerbeschreibungen.keys():
      if fehlerbeschr not in self.epDE.fehlerbeschreibungen.keys():
        raise AssertionError(fehlerbeschr+" nicht in DE")
    pass
  ...
  def testKongENDE(self):
    epen = ExpertenmodellEN()
    epde = ExpertenmodellDE()
    for l in epen.lessons:
      for k in epen.lessoninhalte[l]:
        if(k not in epde.lessoninhalte[l]):
